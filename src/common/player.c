				...
				entity_id_t self, entity_id_t other)
{
	(void)other;
	struct vector *vel =
		get_component(&state->entity_manager, self, velocity_id);
...
{
	entity_id_t player = add_entity(&state->entity_manager);

	struct vector pos = { 200, 200 };
	add_component(&state->entity_manager, player, position_id, &pos);

	global.player.id = player;
	struct vector size = { 20, 20 };
	struct aabb_sprite sprite = { .size = size, .color = cs_orange };
	add_component(&state->entity_manager, player, aabb_sprite_id, &sprite);

	struct aabb_collider collider = { .size = size,
					  .on_collision = player_on_collision,
					  .type = COLL_DYNAMIC };
	add_component(&state->entity_manager, player, aabb_collider_id,
		      &collider);

	struct vector vel = { 0, 0 };
	add_component(&state->entity_manager, player, velocity_id, &vel);
